...
public class JoystickView extends View
    ...
    */
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // Draw the 8-direction guide lines
        Paint guidePaint = new Paint();
        guidePaint.setColor(Color.DKGRAY); // 선 색상 (필요시 변경 가능)
        guidePaint.setStrokeWidth(4f);
        guidePaint.setStyle(Paint.Style.STROKE);
        guidePaint.setAntiAlias(true);

        for (int i = 0; i < 8; i++) {
            double angle = Math.toRadians(i * 45); // 0, 45, ..., 315도
            float endX = (float) (mFixedCenterX + mBorderRadius * Math.cos(angle));
            float endY = (float) (mFixedCenterY + mBorderRadius * Math.sin(angle));
            canvas.drawLine(mFixedCenterX, mFixedCenterY, endX, endY, guidePaint);
        }
